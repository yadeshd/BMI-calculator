...
def selectfile():
    ...
    if file:
        ...
        messagebox.showinfo ("Uploaded", file+" uploadedsuccessfully")
...
def calculate_BMI():
    ...
    if bmi<=18.5:
        ...
        label3.config(text="It indicates that a person is in slightly overweight! In A doctor may advise to lose some \n weight for health reasons!")
...
#icon
image_icon=PhotoImage(file="F:\\Yadesh\\comp coptition\\BMI IMAGE\\icon.png")
root.iconphoto(False,image_icon)

#top
top=PhotoImage(file="F:\\Yadesh\\comp coptition\\BMI IMAGE\\top.png")
...
#two boxes
box=PhotoImage(file="F:\\Yadesh\\comp coptition\\BMI IMAGE\\box.png")
Label(root,image=box).place ( x = 20, y = 100 )
Label(root,image=box).place( x = 240, y = 100 )

#scale
scale=PhotoImage(file="F:\\Yadesh\\comp coptition\\BMI IMAGE\\scale.png")
...
def slider_changed(event):
    ...
    img= (Image.open("F:\\Yadesh\\comp coptition\\BMI IMAGE\\man.jpg"))
...
def slider_changed(event):
    ...
    '''size=int(float (get_current_value()))
    img= (Image.open("F:\\Yadesh\\comp coptition\\BMI IMAGE\\man.png"))
    resized_image=img.resize((50,10+size))
    photo2=ImageTk.PhotoImage(resized_image)
    secondimage.config(image=photo2)
    secondimage.place(x=70,y=550-size)
...
Button(root,text ="View Report",width=15,height=2,font="arial 10 bold",bg="#1f6e68",fg="white", command= calculate_BMI).place(x=280,y=380)
